...
data class MeterReadingUiState(
    ...
    val availableMonths: List<MonthYear> = MonthYear.generateMonthsForYear(MonthYear.current().year),
    val isEditMode: Boolean = false,
    val isLoading: Boolean = true,
    val isSaving: Boolean = false,
    val message: String? = null
) {
    val isCurrentMonth: Boolean get() = selectedMonth == MonthYear.current() // Loosely check if it's the current period
    
    val filteredReadings: List<MeterReadingEntry> get() {
    ...
    val enteredCount: Int get() = readings.values.count { it.currentReading.isNotBlank() }
    val totalConsumption: Long get() = readings.values.sumOf { it.consumption }
}
...
class MeterReadingViewModel(private val repository: AppRepository) : ViewModel() {
    ...
    fun selectYear(year: Int) {
        ...
        val defaultMonth = if (year == MonthYear.current().year) MonthYear.current() else months.first()
    ...
    private fun loadReadingsForZone(zoneId: Long) {
        viewModelScope.launch {
            ...
            repository.getSubscribersByZone(zoneId).collect { subscribers ->
                val entriesMap = subscribers.associate { subscriber ->
                    // 1. Check if invoice already exists for this month
                    val existingInvoice = repository.checkExistingInvoice(subscriber.id, monthStart, monthEnd)
                    
                    if (existingInvoice != null) {
                        // Invoice exists - load it.
                        // We set hasInvoice=true for visual cues, but we also store invoiceId for updates.
                        subscriber.id to MeterReadingEntry(
                            subscriberId = subscriber.id,
                            subscriberName = subscriber.fullName,
                            meterNumber = subscriber.meterNumber,
                            previousReading = existingInvoice.previousReading,
                            currentReading = existingInvoice.currentReading.toString(),
                        ...
                        // 2. Get previous reading from latest invoice
                        val lastInvoice = repository.getLatestInvoiceBySubscriber(subscriber.id)
                        val prevReading = lastInvoice?.currentReading ?: 0L
    ...
    fun toggleEditMode() {
        if (_uiState.value.isCurrentMonth) {
            _uiState.update { it.copy(isEditMode = !it.isEditMode) }
        } else {
            showMessage("لا يمكن التعديل على شهور سابقة")
        }
    }
    ...
    fun updateReading(subscriberId: Long, newReading: String) {
        ...
        val consumption = if (current > currentEntry.previousReading) current - currentEntry.previousReading else 0
        ...
        _uiState.update { state ->
            state.copy(
                readings = state.readings + (subscriberId to updatedEntry)
            )
        }
    }
    ...
    fun saveAllReadings() {
        if (!_uiState.value.isCurrentMonth) {
            showMessage("خطأ: لا يمكن حفظ القراءات لشهر سابق. يرجى اختيار الشهر الحالي.")
            return
        }
        ...
        viewModelScope.launch {
            ...
            try {
                ...
                val entriesToSave = _uiState.value.readings.values.filter { 
                ...
                entriesToSave.forEach { entry ->
                    ...
                    if (entry.invoiceId != null && entry.hasInvoice) {
                        ...
                        // Insert new invoice
                        // Double check one last time before inserting to avoid race condition/duplicates
                        val monthStart = _uiState.value.selectedMonth.startEpochMillis
                        val monthEnd = _uiState.value.selectedMonth.endEpochMillis
                        val exists = repository.checkExistingInvoice(entry.subscriberId, monthStart, monthEnd)
