    ...
    MaterialTheme {
        KoinContext {
            CompositionLocalProvider(LocalLayoutDirection provides LayoutDirection.Rtl) {
                ...
                val isDetailScreen = currentRoute.contains("Detail") || 
                ...
                MainLayout(
                    currentRoute = when {
                        ...
                        currentRoute.contains("Meter") || currentRoute.contains("Reading") -> "readings"
                        currentRoute.contains("Invoice") -> "invoices"
                        currentRoute.contains("Treasury") || currentRoute.contains("Transaction") -> "treasury"
                        currentRoute.contains("Maintenance") || currentRoute.contains("Ticket") -> "maintenance"
                        currentRoute.contains("Settings") || currentRoute.contains("Zone") -> "settings"
                        else -> "dashboard"
                    },
                    onNavigate = { navItem ->
                        when (navItem) {
                            NavItem.Dashboard -> navController.navigate(Screen.Dashboard) {
                                popUpTo(Screen.Dashboard) { inclusive = true }
                            }
                            NavItem.Members -> navController.navigate(Screen.SubscriberList) {
                                popUpTo(Screen.Dashboard)
                            }
                            NavItem.Readings -> navController.navigate(Screen.MeterReading) {
                                popUpTo(Screen.Dashboard)
                            }
                            NavItem.Invoices -> navController.navigate(Screen.InvoicesList) {
                                popUpTo(Screen.Dashboard)
                            }
                            NavItem.Treasury -> navController.navigate(Screen.Treasury) {
                                popUpTo(Screen.Dashboard)
                            }
                            NavItem.Maintenance -> navController.navigate(Screen.MaintenanceList) {
                                popUpTo(Screen.Dashboard)
                            }
                            NavItem.Settings -> navController.navigate(Screen.Settings) {
                                popUpTo(Screen.Dashboard)
                            }
                        }
                    },
