...
data class MonthYear(
    ...
    val startEpochMillis: Long get() {
        val localDate = LocalDate(year, month, 1)
        return localDate.atStartOfDayIn(TimeZone.currentSystemDefault()).toEpochMilliseconds()
    }
    
    val endEpochMillis: Long get() {
        // Simple logic: go to next month/year and subtract 1 ms (or just use day 1 of next month)
        ...
        return nextMonthDate.atStartOfDayIn(TimeZone.currentSystemDefault()).toEpochMilliseconds() - 1
    }
    ...
    companion object {
        ...
        fun getAvailableYears(): List<Int> {
             ...
             // Return current year and previous 4 years (total 5)
             return (currentYear downTo currentYear - 4).toList()
        }
    }
}
