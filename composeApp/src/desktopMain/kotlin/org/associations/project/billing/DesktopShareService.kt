...
class DesktopShareService : ShareService {
    override suspend fun shareInvoice(invoice: Invoice, subscriber: Subscriber, settings: AppSettings) {
        ...
        g2d.font = Font("Dialog", Font.PLAIN, 14)
        g2d.drawString("Invoice #: ${invoice.id}", margin, y)
        g2d.drawString("Date: ${date.date}", margin, y + 20)
        y += 10

        val rightX = imgWidth - margin
        drawRightAlignedText(g2d, subscriber.fullName, rightX, y)
        g2d.font = Font("Dialog", Font.PLAIN, 14)
        drawRightAlignedText(g2d, "Meter: ${subscriber.meterNumber}", rightX, y + 20)
        y += 45
        ...
        g2d.drawString("Current", col1, y)
        g2d.drawString("Previous", col2, y)
        g2d.drawString("Consumption", col3, y)
        y += 5
        g2d.drawLine(margin, y, imgWidth - margin, y)
        y += 20
        ...
        g2d.drawString("${invoice.consumption} m\u00B3", col3, y)
        y += 10
        g2d.drawLine(margin, y, imgWidth - margin, y)
        y += 30
        ...
        drawRightAlignedText(g2d, "${invoice.totalAmount} DH", imgWidth - margin, y)
        y += 50
